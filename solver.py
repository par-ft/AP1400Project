...
class SudokuSolver:
    ...
    def __init__(self, puzzle):
        self.puzzle = puzzle.copy()  # the 2d list of spots on the board
    ...
    def move_forward(self):
        ...
        while self.backtrack_coord < len(self.solve_puzzle) - 1 and self.solve_puzzle[self.backtrack_coord].solved:
            if self.backtrack_coord == 0 and self.checkfirstTime == 0:
    ...
    def CheckForValid(self):
        cell = self.solve_puzzle[self.backtrack_coord]
        if self.checkfirstTime == 1 and self.backtrack_coord == 0 and len(cell.possibilities) == 0:
            print("UNSOLVE")
            self.sul == False
